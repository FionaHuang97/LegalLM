...
#scratch
#r = citation_transformation_log_analysis('../../../data/bva/utils/vocab_transform_log.pkl', raw_vocab=cv_raw)
#with open('../../../data/bva/utils/transform_party_debug.txt', 'w') as f:
#    for raw, transform in r['case_mismatch_party'].items():
#        f.write(f'{raw} | {transform[0]} | {cv_raw.citation_counts[raw]}\n')
...
#scratch
#broken_citation_data = assemble_broken_citation_data(train_ids_fpath, preprocessed_dir, metadata_fpath, r['case_mismatch_party'])
#export_broken_citation_stats(broken_citation_data)


## Once data is preprocessed and a reduced citation vocabulary is obtained, the
## Pytorch dataset object can be created as follows:
# ds = dv.CitationPredictionDataset(preprocessed_dir, cvx, case_ids_fpath=train_ids_fpath)


# # Cache citation_indices to data to speed up training
# with open('../utils/thresholded_vocab.pkl', 'rb') as f:
#     cvx = pickle.load(f)
# parallelize_cache_citation_indices('../utils/cache_indices_ids_1.txt', '../data/preprocessed/', '../data/preprocessed-cached/', cvx)


## =============================================================================
## CLA loader reduction
## =============================================================================

## to be used on vocab object `cv` as follows. Please duplicate the object
# beforehand or back up in pickle
#
# len(cv.transform_fns[2].__self__.cla_citations) # debug print before
# cla_ids = db.get_used_cla_ids(cv)
# db.reduce_cla_to_ids(cv_exp, cla_ids)
# len(cv.transform_fns[2].__self__.cla_citations) # debug print after
