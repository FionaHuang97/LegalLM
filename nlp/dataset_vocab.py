...
#         print(f'unknown %: {unk_count/total_count}')
...
#         print(f'case regex presumed: {case_num} / {case_count}')
...
#     TO DO: This is error prone and can be removed after discussion!
#     def reduce_to_vocab(self, vocab: dv.CitationVocabulary) -> None:
#         '''Reduces the CLA processor to only those case entries that are
#         part of a citation vocabulary. This speeds up processing as
#         citation resolution no longer needs to iterate over all cases
#         in the CLA processor object, but only those that will not resolve
#         to an unknown token'''
#         rex = re.compile(r'CLA#(\d+)$')
#         cla_ids = set()
#         for cit in vocab.citation_counts:
#             m = rex.search(cit)
#             if m:
#                 cla_ids.add(int(m.group(1)))
#         reduced = [c for c in self.cla_citations
#                    if c['id'] in cla_ids]
#         self.cla_citations = reduced
